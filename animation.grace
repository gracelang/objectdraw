...
type Animator = {
   ...
   // Repeatedly execute block while condition is true
   for<T> (range:List<T>) pausing (pauseTime) do (block:Block<T,Done>) -> Done 
 
   // Repeatedly execute block while condition is true
   // when condition fails, execute endBlock.
   for<T> (range:List<T>) pausing (pauseTime) do (block:Block<T,Done>) finally (endBlock:Block) -> Done
...
method for<T>(range:List<T>) pausing (pauseTime: Number) do (block:Block<Number,Done>)-> Done {
...
// block should take a numeric value as a parameter
// when condition fails, execute endBlock.
method for<T> (range:List<T>) pausing (pauseTime) do(block:Block<Number,Done>)
             finally(endBlock:Block) -> Done {
